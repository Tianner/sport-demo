...
<script type="text/javascript" src="/js/jquery-1.6.4.js"></script>
	<script type="text/javascript">
		function goPage(page) {
			...
			if(page>${pageProduct.pages}){
		        page = ${pageProduct.pages};
			}
			window.location.href = "search?pageNum="+page+"&pageSize=${pageProduct.pageSize}&keyword=${keyword}&sortByPrice=${sortByPrice}&brandId=${brandId}&pL=${pL}&pH=${pH}";
        }
        function sortByPrice(sort) {
		    if(sort == 'asc'){
                ...
                window.location.href = "search?pageNum=0&pageSize=${pageProduct.pageSize}&keyword=${keyword}&sortByPrice="+sort+"&brandId=${brandId}&pL=${pL}&pH=${pH}";
        }
        function fqBrand(id) {
            window.location.href = "search?pageNum=${pageProduct.pageNum}&pageSize=${pageProduct.pageSize}&keyword=${keyword}&sortByPrice=${sortByPrice}&brandId="+id+"&pL=${pL}&pH=${pH}";
        }
        function noBrandId() {
            window.location.href = "search?pageNum=1&pageSize=${pageProduct.pageSize}&keyword=${keyword}&sortByPrice=${sortByPrice}&pL=${pL}&pH=${pH}";
        }
        document.onkeydown=function(event){
            var e = event || window.event || arguments.callee.caller.arguments[0];
            if(e && e.keyCode==13){ // enter 键
                window.location.href = "search?pageNum=${pageProduct.pageNum}&pageSize=${pageProduct.pageSize}&keyword="+$("#keyword").val()+"&sortByPrice=${sortByPrice}";
            }
        };
		function fqPrice(pL,pH) {
            window.location.href = "search?pageNum=${pageProduct.pageNum}&pageSize=${pageProduct.pageSize}&keyword="+$("#keyword").val()+"&sortByPrice=${sortByPrice}&brandId=${brandId}&pL="+pL+"&pH="+pH;
        }
        function noPrice() {
            window.location.href = "search?pageNum=${pageProduct.pageNum}&pageSize=${pageProduct.pageSize}&keyword="+$("#keyword").val()+"&sortByPrice=${sortByPrice}&brandId=${brandId}";
        }
	</script>
</head>
<body>
	...
	<div class="crumbs-nav">
		<div class="crumbs-nav-main clearfix">
			...
			<div class="">
				...
				<a href = "javascript:void(0);" class = "crumb-select-item" onclick="noBrandId()" title="取消">
					<strong class="search-key">"${brandName}"</strong>
					<i></i>
				</a>
					<i class = "crumbs-arrow">&gt;</i>
				</c:if>
				<c:if test="${not empty pH && not empty pL}">
					<a href = "javascript:void(0);" class = "crumb-select-item" onclick="noPrice()" title="取消">
						<strong class="search-key">
							<c:if test="${pH == -1}">"${pL}以上"</c:if>
							<c:if test="${pH != -1}">"${pL}-${pH}"</c:if>
						</strong>
						<i></i>
					</a>
					<i class = "crumbs-arrow">&gt;</i>
			...
			<div class="sl-v-list">
				...
				<c:forEach items="${brands }" var="brand">
					<li id="brand-38118" data-initial="j" style="display:block;">
						<a href="javascript:;" onclick="fqBrand('${brand.key}')" title="${brand.value }">${brand.value }</a>
					</li>
				</c:forEach>
				</ul>
			</div>
		...
		<div class="ml-wrap">
			<div id="J_filter" class="filter">
				<div class="f-line top">
					<div class="f-sort"><a href="javascript:;" class="curr">综合排序<i></i></a><a href="javascript:;" class="">销量<i></i></a><a href = "#" onclick="sortByPrice('${sortByPrice}')" class="">价格<i></i></a><a href="javascript:;" class="" >评论数<i></i></a><a href="javascript:;" class="">新品<i></i></a></div>
					<div class="f-search">
						<input type="text" value="在结果中搜索" class="input-txt">
						<a data-type="all" data-field="exp_key" class="btn btn-default" data-url="search?keyword=2016&pageSize=${pageProduct.pageSize}&sortByPrice=${sortByPrice}" href="javascript:;">确定</a>
					</div>
					<div id="J_topPage" class="f-pager"><span class="fp-text"><b><c:if test="${pageProduct.pages == 0}">0</c:if><c:if test="${pageProduct.pages>0}">${pageProduct.pageNum}</c:if></b><em>/</em><i>${pageProduct.pages}</i></span><a class="fp-prev disabled" onclick="goPage('${pageProduct.pageNum-1}')">&lt;</a><a class="fp-next" onclick="goPage('${pageProduct.pageNum+1}')" href="javascript:;" title="使用方向键右键也可翻到下一页哦！">&gt;</a></div>
					<div class="f-result-sum">共<span id="J_resCount" class="num">${pageProduct.total}</span>件商品</div>
					<span class="clr"></span>
				</div>
				<div class="f-line">
					<div class="f-store">
...
<div id="J_goodsList" class="goods-list-v1 gl-type-1 J-goods-list">
	<ul class="gl-warp clearfix" data-tpl="1">
		...
		<li data-sku="1711416562" class="gl-item">
			<div class="gl-i-wrap">
				<div class="p-img">
					<a href="javascript:;" onclick="window.open('product?id=${product.id}')" style="position: relative;">
						<img width="220" height="220" class="err-product"  src="${baseAddr}${product.tian_imgUrl}">
